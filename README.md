...
We recommend using **Python 3.11**. You can manage your Python versions with `pyenv`:
...
Alternatively, you can pull the snapshot data directly from [Zenodo (https://zenodo.org/records/18243205)]([https://sandbox.zenodo.org/records/420119](https://zenodo.org/records/18243205]) using the following command:
...
After the training completes, select the best weight (based on **`checkpoint/student0/mean_minus_std`** metric under MLFlow) from **step 30 onwards** (ignoring earlier epochs due to curriculum learning). Update `params.yaml`:
```yaml
eval:
  policy:
    run_id: <RUN_ID>
...
You can run the associated `dvc.yaml` stages using the following commands:

Thermal localization evaluation:
```shell
dvc repro --single --force eval-peer-informed-student-alone-from-different-distances
dvc repro --single --force eval-peer-informed-student-with-birds-from-different-distances
```

Thermalling efficiency evaluation:
```shell
dvc repro --single --force eval-peer-informed-student-alone-using-bird-wing-loadings-from-close-distance
dvc repro --single --force eval-peer-informed-student-with-birds-using-bird-wing-loadings-from-close-distance
```
...
Generate all of the analysis results:
```
dvc repro --single --force generate_altitude_achievement_percent_by_distance_analysis
dvc repro --single --force generate_close_distance_vertical_velocity_analysis
dvc repro --single --force generate_close_distance_episode_success_analysis
dvc repro --single --force generate_different_distances_wind_speed_analysis
